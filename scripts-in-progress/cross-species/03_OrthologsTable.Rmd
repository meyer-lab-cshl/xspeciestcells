...
If I want to import only expressed features.
...
table(ref.ms$ms_ensemblID %in% biomart.ms$ms_ensemblID) # 213 ms expressed genes not in the biomart / 396
table(ref.hu$hu_symbol_data %in% biomart.ms$hu_symbol) # 5328 hu expressed genes not in the biomart / 12019
table(ref.pig$pig_ensemblID %in% biomart.ms$pig_ensemblID) # 2128 pig expressed genes not in the biomart / 4791
...
ortholog.df <- biomart.ms %>%
  mutate_all(na_if,"") %>%
  filter(hu_orthology_confidence==1 | pig_orthology_confidence==1) %>%
...
# Check for Plzf, 
ortholog.df %>%
  filter(ms_symbol_data %in% c("Zbtb16", "Rorc", "Tbx21"))


# Save
write.csv(ortholog.df, "~/Projects/20220809_Thymic-iNKT-CrossSpecies/data/03_BiomartTable/big_ass_ortholog_table.csv", row.names = F)

# checkup
table(ref.ms$ms_ensemblID %in% ortholog.df$ms_ensemblID) # 5743 ms genes not in the biomart
table(ref.hu$hu_symbol_data %in% ortholog.df$hu_symbol) # 7452 hu genes not in the biomart
table(ref.pig$pig_ensemblID %in% ortholog.df$pig_ensemblID) # 3809 pig genes not in the biomart (out of ~21,000)
```
