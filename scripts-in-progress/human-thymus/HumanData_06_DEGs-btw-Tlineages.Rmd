...
## ****************
## EXPORT TABLES FOR HANNAH
# write.csv(counts.deseq, "~/Projects/HumanThymusProject/data/human-thymus/HumanData_06_DEGs-btw-Tlineages/CD8vsMAIT_thymus_c9c11_rawcountsagg.csv")
# write.csv(metadf.deseq, "~/Projects/HumanThymusProject/data/human-thymus/HumanData_06_DEGs-btw-Tlineages/CD8vsMAIT_thymus_c9c11_metadf.csv")
```
...
counts_corrected <- limma::removeBatchEffect(x=assay(rld),
                                             ...
                                             design=model.matrix(~ cell_id, metadf.deseq))
...
# Output results of Wald test for contrast for CD4 vs NKT
contrast <- c("cell_id", "CD8", "MAIT") # MODIFY BASED ON WHAT YOU ARE COMPARING
res <- results(dds,
               contrast = contrast,
               alpha = 0.05)
res <- lfcShrink(dds,
                 ...
                 contrast = contrast,
                 res=res)
res # condition CD4 vs NKT (high log2FC means upregulated in CD4)
...
head(as.data.frame(result.sig))
```


### 3.3. Heatmap
```{r heatmap, fig.height=8, fig.width=4}
# Don't get normalized counts from DESeq2: these are not batch-corrected!!
...
# Keep only significant DE genes from corrected counts (we corrected the counts with limma a bit earlier)
counts_corrected.sig <- counts_corrected[result.sig$gene,]
dim(counts_corrected.sig)
...
jpeg(file.path(path.plots, "heatmap_c9c10_CD8vsMAIT.jpeg"), height = 1800, width = 1000, res = 300)
pheatmap(counts_corrected.sig,
    color = heat_colors,
    scale = "row", # z-score
    cluster_rows = T,
    cluster_cols = F,
...
## 4.  CELL IDENTITY vs CELL STATE ON PBMC

```{r extract-counts-metadata}
## ****************
## IDENTIFY GROUPS FOR DE ANALYSIS
# Create SingleCellExperiment object
counts <- seur.human@assays$RNA@counts
...
# Correct for batch effect, and then run PCA
counts_corrected <- limma::removeBatchEffect(assay(rld),
                                             batch=metadf.deseq$batch_id,
                                             design=model.matrix(~ cell_id, metadf.deseq))
rv <- matrixStats::rowVars(counts_corrected) # variance of each gene
select_rv <- order(rv, decreasing = TRUE)[seq_len(500)] # get the positions of the top 500 most variable genes?...
...
#### 4.1.3. Run DESeq2
```{r lrt1, fig.width=10, fig.height=10}
## ****************
## RUN DESEQ2
# Run DESeq2 differential expression analysis
...
rld <- rlog(dds, blind=TRUE) # need to provide log-normalized counts to limma
counts.norm.sig <- assay(rld)[rownames(genes.sig),] # keep only the genes of interest
# counts.norm.sig[1:5,1:5]
counts.corr.sig <- t(limma::removeBatchEffect(counts.norm.sig, metadf.deseq$batch_id))
counts.corr.sig[1:5,1:5]
dim(counts.corr.sig) # 1753 genes
...
# Run pheatmap using the metadata data frame for the annotation
path.plots <- "~/Projects/HumanThymusProject/data/human-thymus/HumanData_19_CellIdentity"
# jpeg(file.path(path.plots, "LRT_BatchCell_vs_Batch/heatmap_PBMC_LRTest_batch-cellid_limma_corrected.jpeg"), height = 2000, width = 3500, res = 300)
pheatmap(counts.corr.sig,
    color = heat_colors,
    ...
    show_colnames=F,
    # title
    main="PBMC - LRT test - batchid+cellid vs batchid (batch corrected)")
# dev.off()
```

Try heatmap with binarized counts
```{r heatmap-binarized-counts}
...
# jpeg(file.path(path.plots, "heatmap_PBMC_LRTest_batch-cellid_bin3.jpeg"), height = 1500, width = 2500, res = 300)
pheatmap(test,
    color = c("grey", "black"),
    # color=heat_colors,
    # scale = "column", # z-score
...
#### 4.1.4 Cell lineage-specific upregulated genes
```{r functions}
...
# Plot score
VlnPlot(seur.human, features="CD8sig1", group.by="new_clusters") +
  VlnPlot(seur.human, features="CD4sig2", group.by="new_clusters") +
  VlnPlot(seur.human, features="MAITsig3", group.by="new_clusters") +
  VlnPlot(seur.human, features="NKTsig4", group.by="new_clusters") +
  VlnPlot(seur.human, features="GDTsig5", group.by="new_clusters")
# ggsave(file.path(path.plots, "cellidgenes_scoring2.jpeg"), width=10, height=30)
```


<br/>

### 4.2. LRT on `batch_id + cell_id + cluster_id`
...
# Remove levels that are absent
groups$cluster_id <- factor(groups$cluster_id)
...
counts_corrected <- limma::removeBatchEffect(x=assay(rld),
                                             batch=metadf.deseq$batch_id,
                                             design=model.matrix(~ cell_id + cluster_id, metadf.deseq))
rv <- matrixStats::rowVars(counts_corrected) # variance of each gene
select_rv <- order(rv, decreasing = TRUE)[seq_len(500)] # get the positions of the top 500 most variable genes?...
pca <- prcomp(t(counts_corrected[select_rv,])) # run pca on top 500 HVG
percentVar <- pca$sdev^2/sum(pca$sdev^2)
counts_pca <- data.frame(PC1 = pca$x[, 1], PC2 = pca$x[, 2],
                         ...
                         cell_id = metadf.deseq$cell_id,
                         cluster_id = metadf.deseq$cluster_id) # MODIFY "condition" BASED ON WHAT YOU ARE COMPARING
# Plot
ggplot(counts_pca, aes(x = PC1, y = PC2, color = cell_id, shape=batch_id)) +
  geom_point(size = 4) +
  xlab(paste0("PC1: ", round(percentVar[1] * 100), "% variance")) +
  ylab(paste0("PC2: ", round(percentVar[2] * 100), "% variance")) +
  coord_fixed(expand=TRUE)
...
rld_cor <- cor(counts_corrected[select_rv,])
# Plot heatmap
pheatmap(rld_cor, annotation = metadf.deseq[, c("cell_id", "cluster_id", "batch_id"), drop=F],
        annotation_row=matcol,
        annotation_colors = list(cell_id = col_cellid, batch_id=col_batchid, cluster_id=col_clusterid),)
```


#### 4.2.3. Run DESeq2 (reduced model `~batch_id`)
...
# Output results for contrast batch_id + cell_id + cluster_id VS batch_id
res <- results(dds,
               # contrast = contrast,
               alpha = 0.05)
res <- lfcShrink(dds, type="ashr",
...
dim(genes.sig) # 1164 genes (if padj & log2FC filter), 4796 genes (if padj filter)
...
# Extract normalized counts (from DESeq2 object) of the significant genes
# counts.norm.sig <- t(data.frame(counts(dds, normalized = TRUE))[rownames(genes.sig),])
# dim(counts.norm.sig) # 4796 genes

# Get corrected counts (that we batch corrected with limma earlier)
counts.corr.sig <- t(counts_corrected[rownames(genes.sig),]) # keep only the genes of interest
counts.corr.sig[1:5,1:5]
dim(counts.corr.sig) # 4796 genes (if padj filter)

# Set a color palette
heat_colors <- rev(colorRampPalette(brewer.pal(10, "RdBu"))(100))
# Color for row annotation
matcol <- metadf.deseq %>%
  select(cell_id, batch_id, cluster_id) %>%
  mutate(cell_state=ifelse(cluster_id %in% 9:11, "Tnaive",
                           ifelse(cluster_id==12, "Tcm",
                                  ifelse(cluster_id %in% 13:14, "Th17", "Temra"))))
...
col_clusterid <- c("9" = "#74c8c3", "10" = "#5a97c1", "11" = "gold",
                   "12" = "#a40000", "13" = "#72bcd5", "14" = "grey50",
                   "15" = "orange", "16" = "blueviolet", "17" = "#0a2e57")
col_cellstate <- c("Tnaive"= "#b3e2cd",
                   "Tcm"   = "#f4cae4",
                   "Th17"  = "#cbd5e8",
                   "Temra" = "#fdcdac")
# col_clusterid <- c("9" = "gold", "10" = "gold", "11" = "gold",
#                    "12" = "#a40000", "13" = "#72bcd5", "14" = "#72bcd5",
#                    "15" = "blueviolet", "16" = "blueviolet", "17" = "blueviolet")

# Run pheatmap using the metadata data frame for the annotation
jpeg("../../data/human-thymus/HumanData_19_CellIdentity/LRT_BatchCellCluster_vs_Batch/heatmap_PBMC_LRTest_batch-cellid-clusterid_limma_corrected_adjpval01.jpeg", height = 3000, width = 3500, res = 300)
...
pheatmap(counts.corr.sig,
    ...
    fontsize_row = 10,
    annotation_row=matcol,
    annotation_colors = list(cell_id = col_cellid, batch_id=col_batchid, cluster_id=col_clusterid, cell_state=col_cellstate),
    # Change columns
    angle_col=45,
    # fontsize_col = 1,
    show_colnames=F,
...
# # Are some of the genes present in the GEPs?
# dim(counts.corr.sig)
# test <- data.frame("gene"=colnames(counts.corr.sig))
# test <- test %>%
#   left_join(geps.df %>% mutate("rank"=dplyr::row_number()) %>% pivot_longer(!rank, names_to="GEP", values_to="gene"),
#             by="gene", keep=FALSE) %>%
#   mutate(GEP=replace_na(GEP, "noGEP"),
#          rank=replace_na(rank, 0)) %>%
#   group_by(gene) %>%
#   dplyr::slice(which.min(rank))
# length(unique(test$GEP))
# 
# 
# for(i in 1:13){
#   col_gep <- rep("#f0f0f0", 13)
#   names(col_gep) <- unique(test$GEP)
#   # col_gep[["GEP_1"]] <- "#cbd5e8"
#   # col_gep[["GEP_4"]] <- "#fdcdac"
#   # col_gep[["GEP_5"]] <- "#b3e2cd"
#   # col_gep[["GEP_6"]] <- "#f4cae4"
#   col_gep[i] <- "red"
#   
#   filename <- paste0("../../data/human-thymus/HumanData_19_CellIdentity/LRT_BatchCellCluster_vs_Batch/highlight_gep/heatmap_", names(col_gep)[i], ".jpeg")
#   
#   jpeg(filename, height = 3000, width = 3500, res = 300)
#   pheatmap(counts.corr.sig,
#       color = heat_colors,
#       scale = "column", # z-score
#       cluster_rows = T,
...
#       fontsize_row = 10,
#       annotation_row=matcol,
#       annotation_colors = list(cell_id = col_cellid, batch_id=col_batchid, cluster_id=col_clusterid, cell_state=col_cellstate, GEP=col_gep),
#       annotation_col = as.data.frame(test %>% select(!rank) %>% column_to_rownames(var="gene")),
#       # Change columns
#       angle_col=45,
#       # fontsize_col = 1,
#       show_colnames=F,
...
Get the genes from each dendrogram cluster

```{r}
out <- pheatmap(counts.corr.sig,
    color = heat_colors,
    scale = "column", # z-score
    cluster_rows = T,
    cluster_cols = T,
...
# counts.corr.sig[1:5,1:5]
gene_order <- colnames(counts.corr.sig[,out$tree_col[["order"]]])
tree_numbers <- sort(cutree(out$tree_col, k=10))
unique(unname(tree_numbers[gene_order])) # ORDER OF TREE
table(tree_numbers)

# ATTEMPT AT GENE LISTS
# genes_naive <- names(tree_numbers[tree_numbers==1])
# length(genes_naive) # 1420
genes_Tcm_GEP6 <- names(tree_numbers[tree_numbers==3])
length(genes_Tcm_GEP6) # 188
genes_Th17_GEP1 <- names(tree_numbers[tree_numbers==2])
length(genes_Th17_GEP1) # 521
genes_Temra_GEP4 <- names(tree_numbers[tree_numbers %in% c(7, 4)])
length(genes_Temra_GEP4) # 1506


# Sanity checks
# pheatmap(counts.corr.sig[,genes_naive],
#     color = heat_colors,
#     scale = "column", # z-score
#     cluster_rows = T,
#     cluster_cols = T,
...
geps.df <- read.csv("~/Desktop/genes_per_GEP_df.csv", row.names=1)
table(genes_Tcm_GEP6 %in% geps.df$GEP_6)
table(genes_Th17_GEP1 %in% geps.df$GEP_1)
table(genes_Temra_GEP4 %in% geps.df$GEP_4)
...
path.plots <- "../../data/human-thymus/HumanData_19_CellIdentity/LRT_BatchCellCluster_vs_Batch"
ggsave(plot=compare_with_geps(genesvector=genes_naive, color="#b3e2cd", title="Genes from Tnaive"),
       filename=file.path(path.plots, "comparison_DE_GEPs_naive.jpeg"), width=8, height=5)
ggsave(compare_with_geps(genesvector=genes_Tcm_GEP6, color="#f4cae4", title="Genes from Tcm"),
       filename=file.path(path.plots, "comparison_DE_GEPs_Tcm.jpeg"), width=8, height=5)
ggsave(compare_with_geps(genesvector=genes_Th17_GEP1, color="#cbd5e8", title="Genes from Th17"),
       filename=file.path(path.plots, "comparison_DE_GEPs_Th17.jpeg"), width=8, height=5)
ggsave(compare_with_geps(genesvector=genes_Temra_GEP4, color="#fdcdac", title="Genes from Temra"),
       filename=file.path(path.plots, "comparison_DE_GEPs_Temra.jpeg"), width=8, height=5)
...
Get the genes that are up in naïve vs effector clusters

```{r}
cellStateSignature <- function(cluster, padj_max=0.05, log2FC_min=1){
  
  # All cell types
  allclusters <- 9:17
  allotherclusters <- allclusters[allclusters != cluster]
  ...
  # Loop
  for (clusterid in allotherclusters){
    print(clusterid)
    # Initialize
    out       <- NULL
    result.df <- NULL
    # Set contrast
    contrast <- c("cluster_id", cluster, clusterid)
    # Get results
    out <- results(dds, contrast = contrast, alpha = 0.05)
    out <- lfcShrink(dds, type="ashr", contrast = contrast, res=out)
    # Get log2FC
    ...
    print(paste0("# upregulated genes:", nrow(result.df)))
    # Save DF in list
    list.up[[paste0(cluster, "vs", clusterid)]] <- result.df
...
# Run on cluster 12
c12up <- cellStateSignature(cluster=12, log2FC_min=0.5)
c12up$genesup # 11

res
test <- results(dds,
               contrast = c("cluster_id", 9, 10),
               alpha = 0.05)
test <- lfcShrink(dds, type="ashr",
                 contrast = c("gene_program", "naive", "Tcm_GEP6"),
                 res=test)
table(test$log2FoldChange>1)
colData(dds)$cluster_id
```




#### 4.2.4. Run DESeq2 (reduced model `~batch_id + cell_id`)
```{r, fig.width=10, fig.height=10}
## ****************
## RUN DESEQ2
dds <- DESeqDataSetFromMatrix(counts.deseq, 
                              ...
                              # design = ~ batch_id + cell_id)
                              design = ~ batch_id + cell_id + cluster_id)
dds <- DESeq(dds, test="LRT", reduced=~batch_id + cell_id)

# Plot dispersion estimates
plotDispEsts(dds)

# Output results for contrast batch_id + cell_id + cluster_id VS batch_id + cell_id
res <- results(dds,
               # contrast = contrast,
               alpha = 0.05)
res <- lfcShrink(dds, type="ashr",
...
genes.sig <- res %>%
  ...
  filter(abs(log2FoldChange)>1)
counts.norm.sig <- t(data.frame(counts(dds, normalized = TRUE))[rownames(genes.sig),])
dim(counts.norm.sig) # 4132 genes (with only padj threshold), 1141 genes (with padj+log2FC thresholds)
...
# Run pheatmap using the metadata data frame for the annotation
# jpeg(file.path(path.plots, "LRT_BatchCellCluster_vs_BatchCell/heatmap_PBMC_LRTest_limma_corrected_log2FCabove1_2.jpeg"), height = 3000, width = 3500, res = 300)
pheatmap(counts.corr.sig,
    color = heat_colors,
    scale = "column", # z-score
    cluster_rows = T,
    ...
    show_colnames=F,
    # title
    main="PBMC - LRT test - batchid+cellid+clusterid vs batchid+cellid (batch corrected)")
# dev.off()
```
