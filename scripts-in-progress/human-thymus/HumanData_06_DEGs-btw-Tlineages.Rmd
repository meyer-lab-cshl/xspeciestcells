...
#### 4.1.3. Run DESeq2
```{r lrt1, fig.width=5, fig.height=5}
## ****************
## RUN DESEQ2
# Run DESeq2 differential expression analysis
...
# Run pheatmap using the metadata data frame for the annotation
# jpeg(file.path(path.plots, "LRT_BatchCellCluster_vs_Batch/heatmap_PBMC_LRTest_batch-cellid-clusterid2.jpeg"), height = 3000, width = 3500, res = 300)
pheatmap(counts.norm.sig,
    color = heat_colors,
    scale = "column", # z-score
    cluster_rows = T,
...
genes.sig <- res %>%
  ...
  filter(padj < 0.01)
counts.norm.sig <- t(data.frame(counts(dds, normalized = TRUE))[rownames(genes.sig),])
dim(counts.norm.sig) # 4132 genes

# Set a color palette
heat_colors <- rev(colorRampPalette(brewer.pal(10, "RdBu"))(100))

# Run pheatmap using the metadata data frame for the annotation
# jpeg(file.path(path.plots, "LRT_BatchCellCluster_vs_BatchCell/heatmap_PBMC_LRTest.jpeg"), height = 3000, width = 3500, res = 300)
pheatmap(counts.norm.sig,
    color = heat_colors,
    scale = "column", # z-score
    cluster_rows = T,
    ...
    show_colnames=F,
    # title
    main="PBMC - LRT test - batchid+cellid+clusterid vs batchid+cellid")
# dev.off()
```
