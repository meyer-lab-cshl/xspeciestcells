...
counts.corr.sig[1:5,1:5]
dim(counts.corr.sig) # 1164 genes
...
# Run pheatmap using the metadata data frame for the annotation
# jpeg(file.path(path.plots, "LRT_BatchCellCluster_vs_Batch/heatmap_PBMC_LRTest_batch-cellid-clusterid_limma_corrected_log2FCabove1_1.jpeg"), height = 3000, width = 3500, res = 300)
pheatmap(counts.corr.sig,
    color = heat_colors,
    scale = "column", # z-score
    cluster_rows = T,
...
# Reorder counts.norm.sig by cluster number
order1 <- metadf.deseq %>% arrange(cluster_id, batch_id)
order2 <- metadf.deseq %>% arrange(batch_id, cluster_id)
# counts.norm.sig[1:5,1:5]
counts.norm.sig <- counts.norm.sig[rownames(order1),]
counts.norm.sig <- counts.norm.sig[rownames(order2),]
counts.norm.sig[1:5,1:5]
# jpeg(file.path(path.plots, "LRT_BatchCellCluster_vs_Batch/heatmap_PBMC_LRTest_batch-cellid-clusterid4.jpeg"), height = 3000, width = 3500, res = 300)
pheatmap(counts.norm.sig,
    ...
    show_colnames=F,
    # title
    main="PBMC - LRT test - batchid+cellid+clusterid vs batchid")
# dev.off()
```
...
# Sanity checks
# pheatmap(counts.corr.sig[,genes_Temra_GEP6],
#     color = heat_colors,
#     scale = "column", # z-score
#     cluster_rows = T,
#     cluster_cols = T,
...
test <- geps.df %>%
  ...
  filter(gene %in% genes_Temra_GEP4) %>%
  group_by(GEP) %>%
  summarize(n_gene=n())
ggplot(test, aes(x=GEP, y=n_gene))+
  geom_bar(stat="identity")+
  labs(x="", y="# genes found in GEP", title="Genes up in cluster 15-17")
