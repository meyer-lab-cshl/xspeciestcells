...
# Get union of HVGs
total.hvg <- union(tinn.hvg, tconv.hvg) # 3183 HVGs
...
tot.metadata$cluster <- gsub("-", "_", tot.metadata$cluster)
...
## Export data to run metaneighbor on Elzar
```{r export-data}
seur.total <- CreateSeuratObject(counts=tot.counts, meta.data=tot.metadata) # 14,671 genes by 61,632 cells

# convert to seurat v3 assay to be able to convert
...
SeuratDisk::SaveH5Seurat(
  seur.total,
  filename = "data/mouse-thymus/02_tconv-vs-tinn/elzar/input/ms_tconv_tinn_full_seu_240424.h5Seurat",
  overwrite=T,
  verbose=T
  )
SeuratDisk::Convert(
  source="data/mouse-thymus/02_tconv-vs-tinn/elzar/input/ms_tconv_tinn_full_seu_240424.h5Seurat",
  dest = "h5ad",
  assay="RNA",
  overwrite=T,
  verbose=T
...
write.csv(hvg.df, "data/mouse-thymus/02_tconv-vs-tinn/elzar/input/list_hvg_240424.csv")
```
