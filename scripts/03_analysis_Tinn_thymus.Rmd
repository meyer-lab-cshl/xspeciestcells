...
```{r preprocessing1}
# Preprocessing
preprocess <- function(seurobj, ndim, res, celltype, umapneighbors=50, approxPCA=FALSE){
  print(seurobj)
  ...
  print("UMAP")
  seurobj <- RunUMAP(seurobj, reduction = "harmony", dims = 1:ndim, n.neighbors=umapneighbors, reduction.key = "UMAP50_")
  print("Cluster")
...
### 2.6. GDT cells
```{r preprocessing-gdt}
seur.gd <- preprocess(seurobj = subset(seur.filt, subset=group.ident=="GD_Thymus"), celltype = "GDthy",
                      ndim=15, res=0.4, approxPCA=F, umapneighbors = 50)
...
# Add thymic GDT clusters to integrated object
