...
```{r qc}
# Visualize on UMAP
FeaturePlot(seur.human, features="nFeature_RNA", cols= c("lightgrey", "#67000D")) |
  FeaturePlot(seur.human, features="nCount_RNA", cols= c("lightgrey", "#54278f")) |
  FeaturePlot(seur.human, features="percent.mt", cols= c("lightgrey", "#045a8d"))
# ggsave("~/Projects/HumanThymusProject/data/human-thymus/HumanData_02_ExploreLaurentData/qc_on_umap.jpeg", width=15, height=5)


# QC violin plots
VlnPlot(seur.human, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3, pt.size=.001,
        group.by="group.ident")
# ggsave("~/Projects/HumanThymusProject/data/human-thymus/HumanData_02_ExploreLaurentData/qc_by_group.jpeg", width=12, height=6)
VlnPlot(seur.human, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3, pt.size=.01,
        group.by="Donor")
# ggsave("~/Projects/HumanThymusProject/data/human-thymus/HumanData_02_ExploreLaurentData/qc_by_donor.jpeg", width=12, height=6)
VlnPlot(seur.human, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3, pt.size=.01,
        group.by="Method")
# ggsave("~/Projects/HumanThymusProject/data/human-thymus/HumanData_02_ExploreLaurentData/qc_by_method.jpeg", width=8, height=6)
VlnPlot(seur.human, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3, pt.size=.01,
        group.by="Batch")
# ggsave("~/Projects/HumanThymusProject/data/human-thymus/HumanData_02_ExploreLaurentData/qc_by_batch.jpeg", width=10, height=6)
```
...
# ggsave("~/Projects/HumanThymusProject/data/human-thymus/HumanData_02_ExploreLaurentData/umap_all_cells.jpeg", width=8, height=10)
...
# ggsave("~/Projects/HumanThymusProject/data/human-thymus/HumanData_02_ExploreLaurentData/umap_all_SPthymocytes.jpeg", width=10, height=9)
...
```{r create-shiny-app, echo=TRUE, eval=FALSE}
library(ShinyCell)
...
# ggsave("~/Projects/HumanThymusProject/data/human-thymus/HumanData_02_ExploreLaurentData/hu_umap_iNKT_clust12_recluster.jpeg", width=10, height=9)
...
Compare CD4 and CD8A expression between PLZF+ and PLZF- iNKT cells:
```{r}
# Keep only iNKT cells (thymus & periphery)
nkt <- subset(seur.human, subset= cell.ident=="NKT")

# Add column in metadata to identify PLZF+ and PLZF- cells
nkt@meta.data$PLZF <- ifelse(rownames(nkt@meta.data) %in% WhichCells(nkt, expression= ZBTB16>0), "PLZF+", "PLZF-")
# table(nkt@meta.data$PLZF)

RidgePlot(nkt, features=c("ZBTB16", "CD4", "CD8A"), group.by="PLZF", log=T, stack=T, combine=T)+ labs(x="Expression level (log scale)")
# ggsave("~/Projects/meyerLabNotebook/pelicanNotebook/content/images/2022-10-28_HumanNKT-PLZF/cd4_cd8.jpeg", width=10, height=5)

# Look at DEGs between PLZF+ and PLZF- iNKT cells
FindMarkers(nkt,
            ident.1 = WhichCells(object = nkt, expression = ZBTB16 > 1, slot = 'data'),
            ident.2 = WhichCells(object = nkt, expression = ZBTB16 <= 1, slot = 'data'))
```
...
# ggsave("~/Projects/HumanThymusProject/data/human-thymus/HumanData_02_ExploreLaurentData/hu_fullumap_nktclusters.jpeg", width=5, height=5)
...
DimPlot(seur.human, pt.size = 0.1, label = T, group.by="nkt_clusters_harmony", cells.highlight=nktlist,
  ...
  theme(legend.position="none")+
  ggtitle("Thymic iNKT cells - Clustering on NKT cells only")
# ggsave("~/Projects/HumanThymusProject/data/human-thymus/HumanData_02_ExploreLaurentData/hu_fullumap_nktclusters_harmony.jpeg", width=5, height=5)
```
