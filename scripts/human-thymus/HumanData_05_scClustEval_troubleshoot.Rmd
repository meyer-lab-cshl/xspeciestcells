...
PreprocessSClustEval <- function(seuratobject, variable.features.n=3000, num.pc=50, resolution=0.5, pc.use=15, k.param=20){
  ...
  object <- FindNeighbors(object, dims = 1:pc.use, k.param = k.param, nn.eps = 0,
                               verbose = T, reduction = "harmony", force.recalc = TRUE)
...
troubleshoot <- function(object,
  ...
  set.seed(123)
        ...
        # Extract the counts data (from RNA assay) and metadata
        counts <- GetAssayData(object=object, slot="counts", assay="RNA")
        ...
        if(useSCTransform==TRUE){
                cat("\n1-Perform SCTransform")
                object<- SCTransform(initseurat, vars.to.regress = vars.to.regress,
                                     variable.features.n = variable.features.n, verbose = FALSE, seed.use=1448145)  
        }else{
        ...
        cat("\n2-Run PCA pre-integration with", num.pc, "PCs\n")
        object<- RunPCA(object = object, assay="SCT", features = VariableFeatures(object = object),
                        npcs = num.pc, seed.use=42)
        ...
        # run integration
        cat("\n3-Run Harmony on", pc.use, "PCs\n")
        object <- RunHarmony(object = object, assay.use = "SCT", reduction = "pca", dims.use = 1:pc.use,
                             max.iter.harmony=30, group.by.vars = c("Sex", "Method"), plot_convergence = FALSE)
        cat("\nSeurat object post-integration:\n")
        print(object)
        ...
        object <- FindNeighbors(object, dims = 1:pc.use, k.param = k.param, nn.eps = nn.eps,
                               verbose = TRUE, reduction = "harmony", force.recalc = TRUE)
        object <- FindClusters(object = object,
                                n.start = n.start,
                                resolution = resolution,
                                verbose = TRUE,
...
nkt.seur <- readRDS("../../data/human-thymus/HumanData_05_scClustEval/input/nkt_object_22-11-10.rds")
nkt.localscClust <- troubleshoot(object=nkt.seur, pc.use = 15, k.param = 50, resolution = 0.7)
```
...
```{r import-rds-object}
nkt.k50res07pc15 <- readRDS("~/Projects/HumanThymusProject/data/human-thymus/HumanData_05_scClustEval/troubleshooting/full_sample_k_50_resolution_0.7_PC_15.rds")
