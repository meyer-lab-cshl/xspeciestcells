...
# Identify groups for aggregation of counts (we'll group by cell lineage & batch)
# groups <- colData(sce)[, c("cluster_id", "sample_id")]
groups <- metadata[,c("Tissue", "cell.ident", "Batch")]
colnames(groups) <- c("cluster_id", "group_id", "sample_id")
groups # sanity check


## ****************
## AGGREGATE COUNTS
# Aggregate counts per cluster and batch
count.agg <- Matrix.utils::aggregate.Matrix(t(counts), groupings = groups, fun = "sum") 
# dim(count.agg)
# count.agg[1:6, 1:6]
# table(colSums(count.agg) == 0) # check if any gene has total count of 0

# Split aggregated counts matrix into a list: each element of the list is a cell cluster, and it contains the aggregated counts per batch (rows genes, columns batch)
splitf <- sapply(stringr::str_split(rownames(count.agg), pattern = "_",  n = 2), 
                 `[`, 1) # create a vector that represents how to split samples
count.agg <- split.data.frame(count.agg, factor(splitf)) %>%
  lapply(function(u) magrittr::set_colnames(t(u), stringr::str_extract(rownames(u), "(?<=_)[:graph:]+"))) # create the list
# class(count.agg)
# str(count.agg)
# names(count.agg) # the elements of the list
# head(count.agg[[1]])


## ****************
## SUBSET METADATA & COUNTS TO POPULATION OF INTEREST

# Create metadata
metadf <- groups %>%
  distinct() %>%
  dplyr::rename(batch=sample_id) %>%
  mutate(sample_id=paste(group_id, batch, sep="_"))
rownames(metadf) <- metadf$sample_id
head(metadf)
# Subset the metadata to only the thymocytes
metadf.deseq <- metadf %>%
  filter(cluster_id=="Thymus" & group_id %in% c("CD4", "CD8"))
head(metadf.deseq)

# Subset the counts to only the thymus cells
counts.sub <- count.agg[["Thymus"]]
counts.deseq <- data.frame(counts.sub[, which(colnames(counts.sub) %in% rownames(metadf.deseq))])

# Check that the row names of cluster_metadata are the same as the column names of cluster_counts in order to use as input to DESeq2
metadf.deseq <- metadf.deseq[match(colnames(counts.deseq), metadf.deseq$sample_id),]
...
dds <- DESeqDataSetFromMatrix(counts.deseq, 
                              ...
                              design = ~ group_id + batch) # include the replicate to include batch as another term
```
...
counts_corrected <- limma::removeBatchEffect(assay(rld), metadf.deseq$batch)
...
counts_pca <- data.frame(PC1 = pca$x[, 1], PC2 = pca$x[, 2],
                         batch = metadf.deseq$batch,
                         condition = metadf.deseq$group_id)
# Plot
...
# Plot heatmap
pheatmap(rld_cor, annotation = metadf.deseq[, c("group_id"), drop=F])
```
...
# contrast <- c("group_id", unique(metadf.deseq$group_id)[1], unique(metadf.deseq$group_id)[2])
contrast <- c("group_id", "CD4", "CD8")
res <- results(dds, 
               contrast = contrast,
               alpha = 0.05)
res <- lfcShrink(dds,
...
table(result.sig$log2FoldChange > 0) # 889 upregulated genes in CD4
result.sig %>%
...
pheatmap(counts.norm.sig[,-1], 
    ...
    show_rownames = T,
    fontsize_row = 2,
    annotation = metadf.deseq[, c("batch", "group_id")],
    # annotation_colors= list(batch=c("WT" = "#999999", "KO" = "#ef8a62"),
    #                         group_id=c("KP01"= "#91bfdb", "KP02"="#d8b365")),
    border_color = NA,
    fontsize = 10,
    main= "Thymic cells",
    scale = "row",
