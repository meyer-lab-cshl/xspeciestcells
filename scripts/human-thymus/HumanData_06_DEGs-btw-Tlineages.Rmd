...
groups <- metadata[,c("Tissue", "Batch", "cell.ident")]
colnames(groups) <- c("cluster_id", "sample_id", "group_id")
groups # sanity check
...
metadf <- groups %>%
  ...
  mutate(sample_id=paste(batch, group_id, sep="_"))
...
counts <- count.agg[["Thymus"]]
counts.deseq <- data.frame(counts[, which(colnames(counts) %in% rownames(metadf.deseq))])
...
dds <- DESeqDataSetFromMatrix(counts.deseq, 
                              ...
                              design = ~ group_id) # include the replicate to include batch as another term
```
...
rv <- matrixStats::rowVars(counts_corrected)
select_rv <- order(rv, decreasing = TRUE)[seq_len(500)]
pca <- prcomp(t(counts_corrected[select_rv,]))
...
table(sig_res$log2FoldChange > 0) # 669 upregulated genes in CD4
sig_res %>%
