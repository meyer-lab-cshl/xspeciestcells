...
groups <- metadata[,c("Tissue", "cell.ident", "Batch", "old_clusters")]
colnames(groups) <- c("tissue_id", "cell_id", "batch_id", "cluster_id")
groups # sanity check


## ****************
## AGGREGATE COUNTS
# Aggregate counts per Tissue, cell Lineage, Cluster and Batch => rownames are Tissue_Lineage_Batch_Cluster
...
# Split aggregated counts matrix into a list: each element of the list is a cell cluster, and it contains the aggregated counts per Tissue_Lineage_Batch (rows genes, columns Tissue_Lineage_Batch)
splitf <- sapply(stringr::str_split(rownames(count.agg), pattern = "_",  n = 4), # split the rownames into "Tissue" "Lineage" "Batch" "Cluster"
                 `[`, 4) # keep only the "Cluster"
count.agg <- split.data.frame(count.agg, factor(splitf)) %>%
  lapply(function(u) t(u))
  #lapply(function(u) magrittr::set_colnames(t(u), stringr::str_extract(rownames(u), "(?<=_)[:graph:]+"))) # if you want to remove the cluster number in the conames
# class(count.agg)
# str(count.agg)
# names(count.agg) # the elements of the list => should be the cluster numbers
# head(count.agg[[1]])
...
metadf <- groups %>%
  ...
  mutate(sample_id=paste(tissue_id, cell_id, batch_id, cluster_id, sep="_"))
rownames(metadf) <- metadf$sample_id
head(metadf)
# Subset the metadata to only the population of interest (e.g. cluster 12, all cell lineages but only thymus)
metadf.deseq <- metadf %>%
  filter(cluster_id=="12" & tissue_id=="Thymus" & cell_id %in% c("CD4", "CD8"))
print(metadf.deseq)

# Subset the counts to only the thymus cells
counts.sub <- count.agg[["12"]]
...
dds <- DESeqDataSetFromMatrix(counts.deseq, 
                              ...
                              design = ~ cell_id + batch_id) # include the replicate to include batch as another term
```
...
counts_pca <- data.frame(PC1 = pca$x[, 1], PC2 = pca$x[, 2],
                         ...
                         condition = metadf.deseq$cell_id)
# Plot
...
pheatmap(counts.norm.sig[,-1], 
    ...
    show_rownames = T,
    fontsize_row = 2,
    annotation = metadf.deseq[, c("batch_id", "cell_id")],
