...
# ggsave("~/Projects/HumanThymusProject/data/human-thymus/HumanData_09_UMAP-3D/umap50_3D_3.jpeg", width=12, height=12)
...
# ggsave("~/Projects/HumanThymusProject/data/human-thymus/HumanData_09_UMAP-3D/umap50_3D_4.jpeg", width=12, height=12)
```
...
#### 4.1.5. Regress out TCR signaling genes

```{r regress-out-tcr-signaling}
# Start on QC data
seur.filt <- readRDS("~/Projects/HumanThymusProject/data/CLUSTER/seurat-integration/input/seur_filt_22-12-19.rds")
...
FeaturePlot(test, features="tcr_score1", reduction="UMAP_50") + scale_colour_gradientn(colours = rev(brewer.pal(n = 11, name = "RdBu")))
FeaturePlot(test, features="tcr_score21", reduction="UMAP_50") + scale_colour_gradientn(colours = rev(brewer.pal(n = 11, name = "RdBu")))
VlnPlot(test, features = c("tcr_score1", "tcr_score21"))


# Preprocessing while regressing out TCR signaling genes
...
seur.tcr.out@meta.data$seurat_clusters <- case_when(
  ...
  seur.tcr.out@meta.data$RNA_snn_res.1.1 == 4 ~ 10,
  seur.tcr.out@meta.data$RNA_snn_res.1.1 == 5 ~ 5,
  seur.tcr.out@meta.data$RNA_snn_res.1.1 == 6 ~ 8,
  seur.tcr.out@meta.data$RNA_snn_res.1.1 == 7 ~ 7,
  seur.tcr.out@meta.data$RNA_snn_res.1.1 == 8 ~ 12,
  seur.tcr.out@meta.data$RNA_snn_res.1.1 == 9 ~ 4,
  seur.tcr.out@meta.data$RNA_snn_res.1.1 == 10 ~ 15,
  ...
  seur.tcr.out@meta.data$RNA_snn_res.1.1 == 16 ~ 2
)
...
ggplot(df, aes(y = n, axis1 = laurent, axis2 = tcr_out)) +
  geom_alluvium(aes(fill=laurent), width = 1/12, lode.guidance=" dzigzag") +
  geom_stratum(width = 1/12, fill = "black", color = "grey") +
  geom_label(stat = "stratum", aes(label = after_stat(stratum))) +
