...
-   `aggregate.Matrix` and `dMcast` functions taken from `Matrix.utils`
    package (<https://rdrr.io/cran/Matrix.utils/src/R/Matrix.utils.R>)
-   `prep_data_for_deseq`, `runDeseq` and `runContrast` functions
    written in-house, inspired from tutorial :
...
Here we will keep only alpha beta lineages that are in clusters
3,9,10,11 (the "naive-like" *CCR9*- and *CCR7*-expressing clusters, not
the agonists).
...
```{r}
# we made 4 comparisons, so we need to multiply the pvals by 6
listDE_final_bonf <- lapply(listDE_final, function(x){
  x$padj_bonferroni <- x$padj*6
  return(x)
  }
  )
# listDE_final_bonf$CD4vsCD8

# Get the CD4/iNKT vs CD8/MAIT DE genes
cd4_inkt_up_bonf <- intersect(
  intersect( # up in CD4/iNKT versus CD8
    listDE_final_bonf$CD4vsCD8 %>% filter(log2FoldChange>0 & padj_bonferroni < 0.01) %>% pull(gene),
    listDE_final_bonf$CD8vsiNKT %>% filter(log2FoldChange<0 & padj_bonferroni < 0.01) %>% pull(gene)
  ),
  intersect( # up in CD4/iNKT versus MAIT
    listDE_final_bonf$CD4vsMAIT %>% filter(log2FoldChange>0 & padj_bonferroni < 0.01) %>% pull(gene),
    listDE_final_bonf$iNKTvsMAIT %>% filter(log2FoldChange>0 & padj_bonferroni < 0.01) %>% pull(gene)
  ))

cd8_mait_up_bonf <- intersect(
  intersect( # up in CD8 vs CD4/iNKT
    listDE_final_bonf$CD4vsCD8 %>% filter(log2FoldChange<0 & padj_bonferroni < 0.01) %>% pull(gene),
    listDE_final_bonf$CD8vsiNKT %>% filter(log2FoldChange>0 & padj_bonferroni < 0.01) %>% pull(gene)
  ),
...
DEgenes_to_plot_bonf <- c(cd4_inkt_up_bonf, cd8_mait_up_bonf)


# now get the genes for negative control, in CD4/MAIT vs CD8/iNKT
cd4_mait_up_bonf <- intersect(
  intersect( # up in CD4/MAIT vs CD8
    listDE_final_bonf$CD4vsCD8 %>% filter(log2FoldChange>0 & padj_bonferroni < 0.01) %>% pull(gene),
    listDE_final_bonf$CD8vsMAIT %>% filter(log2FoldChange<0 & padj_bonferroni < 0.01) %>% pull(gene)
  ),
  intersect( # up in CD4/MAIT vs iNKT
    listDE_final_bonf$CD4vsiNKT %>% filter(log2FoldChange>0 & padj_bonferroni < 0.01) %>% pull(gene),
    listDE_final_bonf$iNKTvsMAIT %>% filter(log2FoldChange<0 & padj_bonferroni < 0.01) %>% pull(gene)
  ))

cd8_inkt_up_bonf <- intersect(
  intersect( # up in CD8 vs CD4/MAIT
    listDE_final_bonf$CD4vsCD8 %>% filter(log2FoldChange<0 & padj_bonferroni < 0.01) %>% pull(gene),
    listDE_final_bonf$CD8vsMAIT %>% filter(log2FoldChange>0 & padj_bonferroni < 0.01) %>% pull(gene)
  ),
...
DEgenes_to_plot_negcontrol_bonf <- c(cd4_mait_up_bonf, cd8_inkt_up_bonf)
```
...
# # get gene & tree order
# htmp_gene_order <- rownames(counts_corrected_to_plot[p_out$tree_col[["order"]],])
# htmp_tree_numbers <- sort(cutree(p_out$tree_col, k=4))
# unique(unname(htmp_tree_numbers[htmp_gene_order])) # order of tree
# as.dendrogram(p_out$tree_col)
# 
# # change order to put EPHB6 in-between LRRN3 and ZFAS1
# # htmp_gene_order <- append(htmp_gene_order, values=htmp_gene_order[1], after=which(htmp_gene_order %in% c("LRRN3")))[-1]
# p_out$tree_col$order <- append(p_out$tree_col$order, values=p_out$tree_col$order[1], after=which(htmp_gene_order %in% c("LRRN3")))[-1]
...
```{r plt-dotplot-fig2b, fig.width=10, fig.height=5}
...
## BONFERRONI????
```{r}
listDE_final_fig2c <- listDE_final[!names(listDE_final) %in% c("CD4vsCD8", "iNKTvsMAIT")]
listDE_final_fig2c <- lapply(listDE_final, function(x){
  x$padj_bonferroni <- x$padj*4
  return(x)
  }
  )

cd4_cd8_up_bonf <- intersect(
  intersect( # CD4/CD8 vs iNKT
    listDE_final_fig2c$CD4vsiNKT %>% filter(log2FoldChange>0 & padj_bonferroni < 0.01) %>% pull(gene),
    listDE_final_fig2c$CD8vsiNKT %>% filter(log2FoldChange>0 & padj_bonferroni < 0.01) %>% pull(gene)
  ),
  intersect( # CD4/CD8 vs MAIT
    listDE_final_fig2c$CD4vsMAIT %>% filter(log2FoldChange>0 & padj_bonferroni < 0.01) %>% pull(gene),
    listDE_final_fig2c$CD8vsMAIT %>% filter(log2FoldChange>0 & padj_bonferroni < 0.01) %>% pull(gene)
  ))

inkt_mait_up_bonf <- intersect(
  intersect( # iNKT/MAIT vs CD4
    listDE_final_fig2c$CD4vsiNKT %>% filter(log2FoldChange<0 & padj_bonferroni < 0.01) %>% pull(gene),
    listDE_final_fig2c$CD4vsMAIT %>% filter(log2FoldChange<0 & padj_bonferroni < 0.01) %>% pull(gene)
  ),
...
DEgenes_to_plot_tinn_tconv_bonf <- c(cd4_cd8_up_bonf, inkt_mait_up_bonf)
```
